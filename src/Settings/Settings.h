...
#define MAKE_SETTING(a_type, a_group, a_key, a_default) \
	...
	MAKE_SETTING(bSetting, "PlaceInRedVR", EnableRedPlacement, true);
	MAKE_SETTING(bSetting, "PlaceInRedVR", EnableGroundSnap, true);
	MAKE_SETTING(bSetting, "PlaceInRedVR", EnableObjectSnap, true);
	MAKE_SETTING(bSetting, "PlaceInRedVR", EnableOutlines, true);
...
#undef MAKE_SETTING
	...
	inline void load()
	{
		const auto config = toml::parse_file("Data/F4SE/Plugins/PlaceInRedVR.toml"sv);
...
#define LOAD(a_setting)                                                              \
	...
	if (const auto tweak = config[a_setting.group()][a_setting.key()]; tweak) {      \
		...
		LOAD(EnableRedPlacement);
		LOAD(EnableGroundSnap);
		LOAD(EnableObjectSnap);
		LOAD(EnableOutlines);
